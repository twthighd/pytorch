...
void MetalShaderLibrary::exec_ternary_kernel(TensorIteratorBase& iter,
  ...
  // correct for any strided layout, but only selected by default for the
  // unit-or-broadcast inner family where its loads have locality (see
  // isInnerUnitOrBroadcast); other layouts measure behind the 2D strided
  // kernel. The nonzero output inner stride keeps tile lanes from storing to
  // one address -- unreachable through public ops (TensorIterator rejects
  ...
  bool inner_strided =
      is_structural && !inner_contiguous && isInnerUnitOrBroadcast(iter) && iter.shape()[0] >= INNER_STRIDED_MIN_EXTENT;
